    ...
    try:
        while True:
            if user_input.lower() == "yes":
                # 调用API接口恢复被中断的智能体运行并等待运行完成或再次中断并返回任务ID
                response = resume_agent(user_id, session_id, task_id, "approve")
                break
                ...
                # 调用API接口恢复被中断的智能体运行并等待运行完成或再次中断并返回任务ID
                response = resume_agent(user_id, session_id, task_id,"edit", args={"edited_args": json.loads(new_query)})
                break
            elif user_input.lower() == "message":
                reject_message = Prompt.ask("[highlight]请输入拒绝原因/反馈信息[/highlight]")
                response = resume_agent(user_id, session_id, task_id,"reject", args={"message": reject_message})
                break
            else:
